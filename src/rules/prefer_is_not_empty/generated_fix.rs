...
fn test_prefer_is_not_empty_fix_simple() {
    ...
    LintContext::test_with_parsed_source(bad_code, |context| {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect 'not ... is-empty' pattern"
        );
        ...
        assert_eq!(
            fix.replacements[0].new_text, "$list | is-not-empty",
            "Should convert to is-not-empty"
        );
    });
}
...
fn test_prefer_is_not_empty_fix_variable() {
    ...
    LintContext::test_with_parsed_source(bad_code, |context| {
        ...
        let fix = violation.fix.as_ref().unwrap();
        assert_eq!(
            fix.replacements[0].new_text, "$data | is-not-empty",
            "Should convert to is-not-empty"
        );
    });
}
...
fn test_prefer_is_not_empty_fix_complex_expr() {
    ...
    LintContext::test_with_parsed_source(bad_code, |context| {
        ...
        assert!(
            !violations.is_empty(),
            "Should detect complex expression with 'not ... is-empty'"
        );

        let violation = &violations[0];
        assert!(
            violation.fix.is_some(),
            "Should provide a fix for complex expression"
        );

        let fix = violation.fix.as_ref().unwrap();
        assert!(
            fix.replacements[0].new_text.contains("| is-not-empty"),
            "Should end with is-not-empty"
        );
        assert!(
            fix.replacements[0].new_text.contains("filter"),
            "Should preserve the filter"
        );
    });
}
...
fn test_prefer_is_not_empty_fix_description() {
    ...
    LintContext::test_with_parsed_source(bad_code, |context| {
        ...
        assert!(
            fix.description.contains("Replace"),
            "Fix description should mention replacement"
        );
        assert!(
            fix.description.contains("is-not-empty"),
            "Fix description should mention is-not-empty"
        );
    });
}
...
"#;
    ...
    LintContext::test_with_parsed_source(bad_code, |context| {
        ...
        assert_eq!(
            violations.len(),
            2,
            "Should detect both 'not ... is-empty' patterns"
        );
        ...
        for violation in &violations {
            ...
            assert!(
                fix.replacements[0].new_text.contains("| is-not-empty"),
                "Each fix should use is-not-empty"
            );
        }
    });
}
