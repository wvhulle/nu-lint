...
fn fix_simple_closure() {
    ...
    rule().assert_fix_contains(source, r"$it > 2");
}
...
fn fix_closure_with_line_param() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"$it =~ "pattern""#);
}
...
fn fix_closure_with_field_access() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r"$it.size > 100kb");
}
...
fn fix_closure_with_item_param() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"$it.field == "value""#);
}
...
fn fix_closure_with_pipeline() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r"($it | str length) > 0");
}
...
";
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r"$it > $threshold");
}
...
fn fix_closure_with_complex_condition() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"$it.size > 100kb and $it.type == "file""#);
}
...
fn fix_closure_with_regex() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"$it.name =~ "Car""#);
}
...
fn fix_closure_with_string_operation() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"($it.name | str downcase) =~ "readme""#);
}
...
fn fix_closure_with_math() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r"$it * 2 > 3");
}
...
fn fix_closure_with_date_comparison() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r"$it.modified >= (date now) - 2wk");
}
...
fn fix_multiple_occurrences_of_parameter() {
    let source = r#"ls | where {|f| $f.size > 100kb and $f.type == "file"}"#;
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, "$it.size");
    rule().assert_fix_contains(source, "$it.type");
}
...
fn fix_simple_closure_exact() {
    ...
    let expected = "$it > 2";
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix(source, expected);
}
