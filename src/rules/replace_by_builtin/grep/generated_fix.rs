...
fn fix_grep_with_file_to_where() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"open logs.txt | lines | where $it =~ "error""#);
}
...
fn fix_grep_invert_match() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"open app.log | lines | where $it !~ "debug""#);
...
fn fix_grep_no_file_uses_find() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"find "test""#);
...
fn fix_preserves_pattern() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"where $it =~ "complex[0-9]+""#);
}
...
fn fix_handles_single_quotes() {
    let source = r"^grep 'pattern' file.txt";
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"where $it =~ "pattern""#);
}
...
fn fix_invert_with_count() {
    ...
    rule().assert_violation_count_exact(source, 1);
    rule().assert_fix_contains(source, r#"where $it !~ "debug""#);
    rule().assert_fix_contains(source, "length");
}
