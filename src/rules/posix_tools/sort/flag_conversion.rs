use crate::rules::posix_tools::sort::rule;

#[test]
fn converts_reverse_flag() {
    rule().assert_replacement_contains("^sort -r", "sort --reverse");
    rule().assert_fix_explanation_contains("^sort -r", "--reverse");
}

#[test]
fn converts_numeric_flag() {
    rule().assert_replacement_contains("^sort -n", "sort --natural");
...
fn converts_unique_flag() {
    rule().assert_replacement_contains("^sort -u", "sort");
...
fn converts_key_field() {
    rule().assert_replacement_contains("^sort -k 2", "sort-by 2");
...
fn converts_key_field_compact_format() {
    rule().assert_replacement_contains("^sort -k2", "sort-by 2");
}

#[test]
fn converts_ignore_case_flag() {
    rule().assert_replacement_contains("^sort -f", "sort");
}

#[test]
fn combines_reverse_and_numeric() {
    rule().assert_replacement_contains("^sort -nr", "sort --natural --reverse");
...
fn combines_key_and_reverse() {
    rule().assert_replacement_contains("^sort -k 3 -r", "sort-by 3 --reverse");
}
