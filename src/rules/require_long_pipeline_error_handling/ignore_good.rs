...
fn ignore_pipeline_with_try() {
    rule()
        .assert_ignores("try { ls | where type == file | get name | each { open $in } | to json }");
}

#[test]
fn ignore_pipeline_with_do_ignore() {
    rule()
        .assert_ignores("do -i { ^git status | lines | where { $in | str contains 'modified' } }");
}
...
fn ignore_pipeline_with_error_check() {
    rule().assert_ignores(
        "let result = (^git status | complete); if $result.exit_code == 0 { $result.stdout | \
         lines }",
    );
}
...
fn ignore_pipeline_with_error_make() {
    rule().assert_ignores(
        "ls | get name | each { if not ($in | path exists) { error make { msg: 'File not found' } \
         } }",
    );
}
