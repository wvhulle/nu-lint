...
fn fix_stderr_redirect() {
    ...
    RULE.assert_replacement_contains(source, "^evtest $keyboard e>| ignore | lines");
}
...
fn fix_curl_redirect() {
    ...
    RULE.assert_detects(source);
    RULE.assert_replacement_contains(source, "^curl https://example.com e>| ignore | from json");
}
...
fn fix_grep_redirect() {
    ...
    RULE.assert_detects(source);
    RULE.assert_replacement_contains(source, "^grep 'pattern' file.txt e>| ignore | lines");
}
...
fn fix_stdout_redirect() {
    ...
    RULE.assert_detects(source);
    RULE.assert_replacement_contains(source, "^noisy-cmd o>| ignore | complete | get exit_code");
}
...
fn fix_both_streams_redirect() {
    ...
    RULE.assert_detects(source);
    RULE.assert_replacement_contains(source, "^cmd o+e>| ignore | lines");
}
...
";
    ...
    RULE.assert_replacement_contains(source, "e>| ignore");
}
