...
fn create_violation(match_info: &MatchInfo, _context: &LintContext) -> (Detection, WithDelimiter) {
    ...
    let replacement = generate_parse_replacement(delimiter, &[match_info.index]);
...
impl DetectFix for SplitRowGetMultistatement {
    ...
    fn explanation(&self) -> &'static str {
        "Replace 'split row' followed by indexed 'get' access across statements with 'parse' for structured text extraction"
    }
