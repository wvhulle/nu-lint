...
fn test_fix_curl_with_from_json() {
    ...
    rule().assert_fix_contains(bad_code, "let result =");
    rule().assert_fix_contains(bad_code, "| complete");
    rule().assert_fix_contains(bad_code, "if $result.exit_code != 0");
    rule().assert_fix_contains(bad_code, "error make");
    rule().assert_fix_contains(bad_code, "$result.stdout");
}
...
fn test_fix_grep_with_lines() {
    ...
    rule().assert_fix_contains(bad_code, "let result =");
    rule().assert_fix_contains(bad_code, "^grep 'pattern' file.txt | lines | complete");
    rule().assert_fix_contains(bad_code, "if $result.exit_code != 0");
}
