...
fn file_operation_ignore_pattern() -> &'static Regex {
    ...
    PATTERN.get_or_init(|| {
        ...
        // This catches the common mistake where users think | ignore will suppress errors
            ...
            if is_external {
                ...
                Some(
                    RuleViolation::new_dynamic(
                        ...
                        "Using '| ignore' with file operations doesn't suppress errors - use 'do -i { ... }' instead".to_string(),
                        nu_protocol::Span::new(ignore_start, ignore_end),
                    )
                    .with_suggestion_static(
                        "'| ignore' only discards output, not errors. For error suppression, wrap in 'do -i { ... }' or use try-catch",
                    ),
                )
            }
        })
