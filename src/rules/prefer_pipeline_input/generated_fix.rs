...
fn fix_simple_each_operation() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def process-items [] { each { |x| $x * 2 } }",
    );
    rule().assert_suggestion_contains(
        source,
...
fn fix_where_operation() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def filter-positive [] { where $it > 0 }",
    );
    rule().assert_suggestion_contains(
        source,
...
fn fix_select_operation() {
    ...
    rule().assert_detects(source);
    rule().assert_suggestion_contains(source, "Change to: def get-names [] { select name }");
    rule().assert_suggestion_contains(
        source,
...
fn fix_sort_by_operation() {
    ...
    rule().assert_detects(source);
    rule().assert_suggestion_contains(source, "Change to: def sort-by-name [] { sort-by name }");
    rule().assert_suggestion_contains(
        source,
        "Remove the 'items' parameter and use pipeline input",
    );
...
fn fix_group_by_operation() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def group-items [] { group-by category }",
    );
...
fn fix_reduce_operation() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def sum-values [] { reduce { |acc, val| $acc + $val } }",
    );
    rule().assert_suggestion_contains(
        source,
        "Remove the 'numbers' parameter and use pipeline input",
...
fn fix_math_operations() {
    ...
    rule().assert_detects(source);
    rule().assert_suggestion_contains(source, "Change to: def sum-all [] { math sum }");
    rule().assert_suggestion_contains(
        source,
        "Remove the 'numbers' parameter and use pipeline input",
    );
...
fn fix_length_operation() {
    ...
    rule().assert_detects(source);
    rule().assert_suggestion_contains(source, "Change to: def count-items [] { length }");
    rule().assert_suggestion_contains(source, "Remove the 'data' parameter and use pipeline input");
}
...
fn fix_typed_list_parameter() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def process-list [] { each { |x| $x + 1 } }",
    );
    rule().assert_suggestion_contains(
        source,
        "Remove the 'items' parameter and use pipeline input",
...
fn fix_typed_table_parameter() {
    ...
    rule().assert_suggestion_contains(
        source,
        "Change to: def process-table [] { select name age }",
    );
    rule().assert_suggestion_contains(source, "Remove the 'data' parameter and use pipeline input");
}
...
fn fix_string_data_processing() {
    ...
    rule().assert_detects(source);
    rule().assert_suggestion_contains(source, "Change to: def split-lines [] { lines }");
