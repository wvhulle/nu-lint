...
impl MultilineFormatting {
    fn complex_construct_pattern() -> &'static Regex {
        ...
        PATTERN.get_or_init(|| {
            Regex::new(
                r"(?m)^\s*(?:def|export def|if|match|for|while|try)\s+[^{]*\{[^}]*\}.*$"
            ).unwrap()
        })
    }
    ...
    fn check(&self, context: &LintContext) -> Vec<Violation> {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            if Self::complex_construct_pattern().is_match(line) && line.len() > 80 {
                let line_start = source[..].lines().take(line_num).map(|l| l.len() + 1).sum::<usize>();
                violations.push(Violation {
                    rule_id: self.id().to_string(),
                    severity: self.severity(),
                    message: "Complex constructs should use multi-line format for better readability".to_string(),
                    span: nu_protocol::Span::new(line_start, line_start + line.len()),
                    suggestion: Some("Consider breaking this construct across multiple lines".to_string()),
                    fix: None,
                    file: None,
                });
            }
            ...
            if Self::multiline_list_pattern().is_match(line) && line.len() > 60 {
                ...
                violations.push(Violation {
                    rule_id: self.id().to_string(),
                    severity: self.severity(),
                    message: "Long lists should use multi-line format with each item on a separate line".to_string(),
                    span: nu_protocol::Span::new(line_start, line_start + line.len()),
            ...
            if Self::multiline_record_pattern().is_match(line) && line.len() > 60 {
                ...
                violations.push(Violation {
                    rule_id: self.id().to_string(),
                    severity: self.severity(),
                    message: "Long records should use multi-line format with each field on a separate line".to_string(),
                    span: nu_protocol::Span::new(line_start, line_start + line.len()),
