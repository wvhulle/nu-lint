...
";
        ...
        assert_eq!(
            rule.check(&context).len(),
            0,
            "Should not flag for loops with external tools"
        );
    }
...
";
        ...
        assert_eq!(
            rule.check(&context).len(),
            0,
            "Should not flag for loops with file operations"
        );
    }

    #[test]
    fn test_for_loop_with_system_commands_not_flagged() {
        let rule = PreferEachOverFor::new();
...
"#;
        ...
        assert_eq!(
            rule.check(&context).len(),
            0,
            "Should not flag for loops with system commands"
        );
    }
