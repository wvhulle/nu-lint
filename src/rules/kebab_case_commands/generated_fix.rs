...
mod tests {
    use crate::{
        context::LintContext, rule::AstRule, rules::kebab_case_commands::KebabCaseCommands,
    };
    ...
    fn test_kebab_case_fix_camel_case() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            ...
            assert_eq!(
                fix.replacements[0].new_text, "my-command",
    ...
    fn test_kebab_case_fix_export_def() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            ...
            assert!(
                !violations.is_empty(),
                "Should detect camelCase export command"
            );

            let violation = &violations[0];
            assert!(
                violation.fix.is_some(),
                "Should provide a fix for export def"
            );
