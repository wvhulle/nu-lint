...
mod tests {
    ...
    fn test_pipe_spacing_fix_no_spaces() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            ...
            assert_eq!(fix.replacements[0].new_text, " | ", "Should replace with proper spacing");
        });
    }
    ...
    fn test_pipe_spacing_fix_no_space_before() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            let violations = rule.check(&context);
            assert!(!violations.is_empty(), "Should detect pipe without space before");
    ...
    fn test_pipe_spacing_fix_no_space_after() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            let violations = rule.check(&context);
            assert!(!violations.is_empty(), "Should detect pipe without space after");
    ...
    fn test_pipe_spacing_fix_multiple_spaces() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            let violations = rule.check(&context);
            assert!(!violations.is_empty(), "Should detect pipe with multiple spaces");
    ...
    fn test_pipe_spacing_fix_description() {
        ...
        LintContext::test_with_parsed_source(bad_code, |context| {
            ...
            assert!(fix.description.contains("Fix pipe spacing"), "Fix description should mention pipe spacing");
        });
    }
}